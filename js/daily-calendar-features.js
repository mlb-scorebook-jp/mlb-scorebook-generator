...
(() => {
    ...
    const renderBirthdayCard = (player, date, roster) => {
        ...
        if (!totals.hitting.G && !totals.pitching.G) blocks.append(empty("集計範囲内の誕生日出場なし"));
        ...
        const coverage = document.createElement("small"); coverage.textContent = "通算集計：1964–2021年レギュラーシーズン";
    ...
    const render = async (date) => {
        ...
        if (activePlayers.length) activePlayers.forEach(({ player, roster }) =>
            birthday.content.append(renderBirthdayCard(player, date, roster)));
