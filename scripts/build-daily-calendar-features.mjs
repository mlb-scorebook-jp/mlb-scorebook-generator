...
for (let start = 0; start < missingIds.length; start += 50) {
    ...
    process.stdout.write(`\rPeople ${Math.min(start + ids.length, missingIds.length)}/${missingIds.length}`);
}
if (missingIds.length) process.stdout.write("\n");
...
for (const [monthDay, players] of [...byMonthDay].sort()) {
    ...
    await writeJson(output, { schemaVersion: 1, monthDay, statsRange: { start: 1964, end: 2021,
        gameTypes: ["R"] }, players });
    await writeDataScript(output.replace(/\.json$/, ".js"), `birthday:${monthDay}`,
        { schemaVersion: 1, monthDay, statsRange: { start: 1964, end: 2021, gameTypes: ["R"] }, players });
...
const report = {
    ...
    statsRange: { start: 1964, end: 2021, gameTypes: ["R"] },
